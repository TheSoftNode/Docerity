...
test.describe("Mentorship", () => {
  test("renders the growth path with all milestones and no console errors", async ({ page }) => {
    const errors: string[] = [];
    page.on("console", (msg) => {
    ...
    for (const label of ["Foundations", "Code Reviews", "System Design", "Leadership"]) {
      await expect(page.locator("#mentorship").getByText(label, { exact: true })).toBeVisible();
    }
