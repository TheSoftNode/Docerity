...
function BlogArticle({ entry }: { entry: BlogEntry }) {
  ...
  return (
    <>
      ...
      <section className="bg-card pt-10 pb-28 sm:pt-14 sm:pb-32">
        <Container>
          <div className="mx-auto max-w-4xl">
            <div className="flex flex-col gap-4 border-b border-border/80 pb-8 sm:flex-row sm:items-center sm:gap-5">
              {isExplainer ? (
                <div className="flex shrink-0 items-center gap-2">
                  <span className="flex size-11 items-center justify-center rounded-xl border border-border/80 bg-card">
                    <entry.concept.Icon className="size-5 text-foreground" strokeWidth={1.75} />
                  </span>
                ...
                <span className="flex size-11 shrink-0 items-center justify-center rounded-xl border border-border/80 bg-card">
                  <entry.Icon className="size-5 text-foreground" strokeWidth={1.75} />
                </span>
              )}
            ...
            <ReadingProgress>
              <div className="mt-10">
                {isExplainer ? (
                  <div className="grid grid-cols-1 gap-x-12 gap-y-10 lg:grid-cols-[1fr_300px]">
            ...
            <div className="mx-auto mt-14 flex max-w-2xl flex-col items-center gap-4 border-t border-border/80 pt-10 text-center">
            ...
            {(prevPost || nextPost) && (
              <div className="mx-auto mt-14 grid max-w-2xl grid-cols-2 gap-4 border-t border-border/80 pt-10">
                <div>
                  {prevPost && (
                    <Link
                      href={`/blog/${prevPost.slug}`}
                      className="group flex flex-col gap-1 rounded-xl border border-border/80 bg-card p-4 transition-colors hover:bg-muted"
                    >
                ...
                <div>
                  {nextPost && (
                    <Link
                      ...
                      className="group flex flex-col items-end gap-1 rounded-xl border border-border/80 bg-card p-4 text-right transition-colors hover:bg-muted"
                    >
                      <span className="flex items-center gap-1 text-xs text-muted-foreground">
                        Next
            ...
            {related.length > 0 && (
              <div className="mx-auto mt-14 max-w-2xl border-t border-border/80 pt-10">
                <p className="font-mono text-xs tracking-[0.2em] text-primary uppercase">
                ...
                <div className="mt-5 grid gap-4 sm:grid-cols-2">
                  {related.map((relatedEntry) => (
                    <Link
                      ...
                      className="flex items-center gap-4 rounded-xl border border-border/80 bg-card p-4 transition-colors hover:bg-muted"
                    >
                      <span className="flex size-11 shrink-0 items-center justify-center rounded-xl border border-border/80 bg-card">
