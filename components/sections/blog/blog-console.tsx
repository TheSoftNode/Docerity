  ...
  return (
    <div className="flex items-center gap-1.5">
      {[0, 1, 2].map((i) => (
        <motion.span
          key={i}
          className="size-1 rounded-full bg-[#070b16]/40"
          animate={
            reduceMotion
              ? { opacity: 0.4 }
  ...
  return (
    ...
    >
      <div className="flex items-center gap-4">
        <span className="flex size-14 shrink-0 items-center justify-center rounded-2xl border border-[#070b16]/15 bg-[#f5f7fb]">
          <post.concept.Icon className="size-6 text-[#070b16]" strokeWidth={1.75} />
        </span>
      ...
      <p className="mt-5 font-mono text-xs tracking-[0.15em] text-[#070b16]/50 uppercase">
        {post.concept.label} &middot; explained as
      </p>
      <h3 className="mt-1 font-heading text-2xl font-medium text-[#070b16] sm:text-3xl">
        {post.analogy.label}
      </h3>
      <p className="mt-3 max-w-md text-sm leading-relaxed text-[#070b16]/70 sm:text-base">
        {post.hook}
      </p>
      ...
      <div className="mt-6 flex items-center gap-4">
        ...
        <span className="text-xs text-[#070b16]/50">{post.readTime}</span>
      </div>
    </motion.div>
  );
}
...
function ArticleDetail({ post }: { post: ArticlePost }) {
  return (
    ...
    >
      <span className="flex size-14 shrink-0 items-center justify-center rounded-2xl border border-[#070b16]/15 bg-[#f5f7fb]">
        <post.Icon className="size-6 text-[#070b16]" strokeWidth={1.75} />
      </span>
...
function BlogConsole() {
  ...
  return (
    <div className="flex flex-1 flex-col overflow-hidden rounded-2xl border border-[#070b16]/10 bg-[#f5f7fb] lg:min-h-0">
      <div className="flex flex-wrap items-center justify-between gap-3 border-b border-[#070b16]/10 px-6 py-4">
        <p className="font-mono text-xs tracking-[0.2em] text-[#070b16]/60 uppercase">
          Tech Explainers &middot; Blog
        </p>
        <div className="flex items-center gap-1 rounded-full bg-[#070b16]/[0.05] p-1">
          {tabs.map((tab) => (
            <button
              key={tab.value}
              type="button"
              ...
              className={cn(
                ...
                activeTab === tab.value
                  ...
                  : "text-[#070b16]/60 hover:text-[#070b16]"
              )}
            >
              {tab.label}
            </button>
      ...
      <div className="hidden flex-1 lg:grid lg:min-h-0 lg:grid-cols-[0.85fr_1.15fr]">
        <nav className="flex flex-col justify-center divide-y divide-[#070b16]/10 overflow-y-auto border-r border-[#070b16]/10">
          {list.map((entry) => {
            ...
            return (
              <button
                ...
                className={cn(
                  ...
                  isActive ? "bg-[#070b16]/[0.04]" : "hover:bg-[#070b16]/[0.02]"
                )}
              >
                <span className="flex items-center gap-2">
                  ...
                  />
                  <span className="font-mono text-sm font-medium text-[#070b16]">
                    {entry.type === "explainer" ? entry.concept.label : entry.title}
                  </span>
                </span>
                <span className="pl-3.5 text-xs text-[#070b16]/50">
      ...
      <div className="flex flex-col gap-3 p-4 lg:hidden">
        {list.map((entry) => (
          <Link
            ...
            className="flex items-center gap-4 rounded-xl border border-[#070b16]/10 bg-white/60 p-4"
          >
            ...
            <ArrowRightIcon className="size-4 shrink-0 text-[#070b16]/40" />
          </Link>
        ))}
      </div>
    </div>
